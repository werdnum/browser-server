...
- `blob` is the scope-filtered Playwright `storage_state` JSON encrypted with AES-256-GCM (Fernet is
  ...
  as a secret alongside `BROWSER_HANDOFF_SERVICE_TOKEN`). Each blob records the `key_id` (a short
  fingerprint of the key that encrypted it) so the service can distinguish "operator rotated the key"
  from "blob corrupted", and so rotation can be handled deliberately.
...
- **`confine_navigation`** (default `true` for jar-loaded sessions): restricts main-frame document
  ...
  domains (see "top-level navigation confinement" below). "Exact origin" means **scheme + host +
  port** (per the web origin definition) — a jar for `https://example.com` does not reach
  `https://example.com:8443`. The Family Assistant side requires this
...
- `models.py`: `CreateSessionRequest` gains `jar_id`/`allow_exec`/`confine_navigation`;
  `BrowserSession` gains `jar_id`/`jar_origins`/`jar_nav_allowlist`/`jar_registrable_domains`; new jar
  request/response models (the save request carries `nav_allowlist`/`storage`/`probe`). **The
